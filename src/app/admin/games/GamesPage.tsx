...
function formatSavedScore(match: any) {
  ...
  const result = String(
    match.result ?? match.score ?? match.final_score ?? "",
  ).trim();
...
function getGameGiftIssued(game: Pick<GameLink, "gifts" | "giftsIssued">) {
  if (Number.isFinite(game.giftsIssued))
    return Math.max(0, Number(game.giftsIssued));
  const parsed = Number(String(game.gifts ?? "").replace(/[^0-9.]/g, ""));
  return Number.isFinite(parsed) ? parsed : 0;
}

function formatMoney(value: number) {
  const safe = Number.isFinite(value) ? Math.max(0, value) : 0;
  return safe > 0 ? `$${safe.toLocaleString()}` : "$0";
}
  ...
  return matches
    .map((match: any) => {
      ...
      const closeMs = new Date(
        match.closes_at ?? match.close_at ?? "",
      ).getTime();
      ...
      const status =
        isClosedByStatus || match.is_active === false
          ? "Closed"
      ...
      return {
        ...
        status,
        players: Number(
          match.entries_count ?? match.players_count ?? match.players ?? 0,
        ),
        score: formatSavedScore(match),
        gifts: formatGiftSummary(match),
        giftsIssued:
          Number(
            match.gifts_issued_count ??
              match.gifts_count ??
              match.gift_count ??
              match.gifts_sent_count ??
              match.rewards_count ??
        ...
        giftsRedeemed:
          Number(
            match.gifts_redeemed_count ?? match.redeemed_gifts_count ?? 0,
          ) || 0,
        giftValue: Number(match.gifts_value ?? match.gift_value ?? 0) || 0,
        giftRedeemedValue:
          Number(
            match.gifts_redeemed_value ?? match.redeemed_gifts_value ?? 0,
          ) || 0,
        tournamentId:
          match.tournament_id ?? match.prediction_tournaments?.id ?? null,
        tournamentName:
...
export function GamesPage({
  ...
  const [tournamentForm, setTournamentForm] = useState({
    name: "",
    sport_type: "basketball" as "football" | "basketball",
  });
  const [tournamentSaving, setTournamentSaving] = useState(false);
  const [tournamentDeleteId, setTournamentDeleteId] = useState<string | null>(
    null,
  );

  // Games list
  const [createdGameLinks, setCreatedGameLinks] = useState<GameLink[]>(() =>
    normalizeGameLinks(initialMatches),
  );
  const [totalWebsiteUsers, setTotalWebsiteUsers] = useState(0);
  const [websiteProfiles, setWebsiteProfiles] = useState<any[]>([]);
  const [gameParticipants, setGameParticipants] = useState<
    Record<string, string[]>
  >({});
  const [globalGamePlayerIds, setGlobalGamePlayerIds] = useState<string[]>([]);
  const [gameDateFilter, setGameDateFilter] = useState<
    "all" | "today" | "week" | "month" | "ended"
  >("today");
  const [gameSportFilter, setGameSportFilter] = useState<
  ...
  async function updateFootballScores() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          json?.error || json?.message || "Could not update scores.",
        );
      }
      ...
      flash(
        saved > 0
          ? `Updated ${saved} score${saved === 1 ? "" : "s"}.`
          : `Checked ${checked} games. No new finished scores yet.`,
      );
    } catch (error) {
      flash(
        error instanceof Error ? error.message : "Could not update scores.",
        "error",
      );
    } finally {
  ...
  async function refreshGameLinks() {
    try {
      const loaded: any[] = [];
      const apiUrls = [
        "/api/admin/prediction-matches",
        "/api/prediction-matches",
        "/api/admin/game-links",
      ];
      ...
      for (const url of apiUrls) {
        try {
          ...
          if (!res.ok) continue;
          const rows = Array.isArray(json.matches)
            ? json.matches
      ...
      setWebsiteProfiles(loadedProfiles);
      setTotalWebsiteUsers(loadedProfileCount);

      const ids = rows.map((row) => String(row.id ?? "")).filter(Boolean);
      const playerCounts = new Map<string, Set<string>>();
      const allGlobalPlayerIds = new Set<string>();

      function addPlayer(matchId: string, rawPlayerId: unknown) {
        const playerId = String(rawPlayerId ?? "").trim();
        if (!playerId || playerId === "null" || playerId === "undefined") return;
        allGlobalPlayerIds.add(playerId);
        if (!matchId) return;
        if (!playerCounts.has(matchId)) playerCounts.set(matchId, new Set());
        playerCounts.get(matchId)?.add(playerId);
      }

      async function collectPlayerCounts(
        table: string,
        matchColumn: "match_id" | "prediction_match_id" | "prediction_game_id" | "game_id",
        playerColumn: "client_id" | "profile_id" | "user_id" | "player_id",
      ) {
        if (ids.length === 0) return;

        const { data, error } = await supabase
          .from(table)
          .select(`${matchColumn}, ${playerColumn}`)
          .in(matchColumn, ids)
          .limit(10000);
        ...
        for (const entry of data as any[]) {
          addPlayer(String(entry?.[matchColumn] ?? ""), entry?.[playerColumn]);
        }
      }

      async function collectGlobalPlayers(
        table: string,
        playerColumn: "client_id" | "profile_id" | "user_id" | "player_id",
      ) {
        const { data, error } = await supabase
          .from(table)
          .select(playerColumn)
          .not(playerColumn, "is", null)
          .limit(10000);

        if (error || !data) return;

        for (const entry of data as any[]) {
          addPlayer("", entry?.[playerColumn]);
        }
      }

      const predictionTables = ["prediction_entries", "game_predictions", "predictions"];
      const matchColumns = ["match_id", "prediction_match_id", "prediction_game_id", "game_id"] as const;
      const playerColumns = ["client_id", "profile_id", "user_id", "player_id"] as const;

      for (const table of predictionTables) {
        for (const matchColumn of matchColumns) {
          for (const playerColumn of playerColumns) {
            await collectPlayerCounts(table, matchColumn, playerColumn);
          }
        }
        for (const playerColumn of playerColumns) {
          await collectGlobalPlayers(table, playerColumn);
        }
      }

      setGameParticipants(
        Object.fromEntries(
          Array.from(playerCounts.entries()).map(([matchId, clients]) => [
            matchId,
            Array.from(clients),
          ]),
        ),
      );
      setGlobalGamePlayerIds(Array.from(allGlobalPlayerIds));
      ...
      function addGiftStat(matchId: string, reward?: any) {
        if (!matchId) return;
        const current = giftStats.get(matchId) ?? {
          issued: 0,
          redeemed: 0,
          value: 0,
          redeemedValue: 0,
        ...
        const rawStatus = String(
          reward?.reward_status ?? reward?.status ?? "",
        ).toLowerCase();
        const rawValue = Number(
          reward?.gift_value ?? reward?.value ?? reward?.amount ?? 0,
        );
        const value = Number.isFinite(rawValue) ? Math.max(0, rawValue) : 0;
        current.issued += 1;
        current.value += value;
        if (["redeemed", "used", "claimed"].includes(rawStatus)) {
          current.redeemed += 1;
          current.redeemedValue += value;
        }
        giftStats.set(matchId, current);
      }
      ...
      async function collectGiftCountsFromMatchColumn(column: string) {
        ...
        const rich = await supabase
          .from("rewards")
          .select(`id, ${column}, reward_status, status, gift_value, value, amount`)
          .in(column, ids);
      ...
      async function collectGiftCountsFromDescriptionMarker() {
        ...
        const { data, error } = await supabase
          .from("rewards")
          .select("id, description, reward_type, reward_status, status, gift_value, value, amount")
      ...
      const withCounts = rows.map((row) => ({
        ...row,
        entries_count:
          playerCounts.get(String(row.id))?.size ??
          row.entries_count ??
          row.players_count ??
          row.players ??
          0,
        players_count:
          ...
          0,
        gifts_count:
          giftStats.get(String(row.id))?.issued ??
          row.gifts_count ??
          row.gift_count ??
          row.gifts_sent_count ??
          row.rewards_count ??
          row.rewards_created ??
          0,
        gifts_issued_count:
          giftStats.get(String(row.id))?.issued ?? row.gifts_issued_count ?? 0,
        gifts_redeemed_count:
          giftStats.get(String(row.id))?.redeemed ??
          row.gifts_redeemed_count ??
          0,
        gifts_value:
          giftStats.get(String(row.id))?.value ?? row.gifts_value ?? 0,
        gifts_redeemed_value:
          giftStats.get(String(row.id))?.redeemedValue ??
          row.gifts_redeemed_value ??
          0,
      }));
  ...
  async function refreshTournaments() {
    try {
      ...
      const text = await res.text();
      const json = text
        ? (JSON.parse(text) as { tournaments?: Tournament[]; error?: string })
        : {};
      if (!res.ok) {
  ...
  async function deleteTournament(id: string) {
    try {
      const res = await fetch(
        `/api/admin/prediction-tournaments?id=${encodeURIComponent(id)}`,
        { method: "DELETE" },
      );
      const json = (await res.json()) as { success?: boolean; error?: string };
      if (!res.ok || !json.success) {
        flash(json.error ?? "Could not delete.", "error");
        return;
      }
      setPredictionTournaments((cur) => cur.filter((t) => t.id !== id));
      setTournamentDeleteId(null);
      setGameForm((cur) =>
        cur.tournament_id === id ? { ...cur, tournament_id: "" } : cur,
      );
  ...
  useEffect(() => {
    ...
    const selected = predictionTournaments.find(
      (t) => t.id === gameForm.tournament_id,
    );
  ...
  function setKickoffWithDefaultWindow(value: string) {
    setGameForm((cur) => {
      if (!value)
        return { ...cur, kickoff_at: "", opens_at: "", closes_at: "" };
      ...
      return {
        ...cur,
        kickoff_at: value,
        opens_at: fmt(new Date(kickoff.getTime() - 20 * 60 * 1000)),
        closes_at: fmt(new Date(kickoff.getTime() + 10 * 60 * 1000)),
      };
    });
  }
  ...
  async function createGame() {
    ...
    try {
      ...
      const text = await res.text();
      const json = text
        ? (JSON.parse(text) as { match?: any; error?: string })
        : {};
      if (!res.ok || !json.match) {
        flash(json.error ?? "Could not create game.", "error");
        return;
      }
      await refreshGameLinks();
      setGameForm({
  ...
  async function downloadAllQrs() {
    if (sortedGameLinks.length === 0) {
      flash("No games to download.", "error");
      return;
    }
    const tournamentName =
      gameTournamentFilter !== "all"
        ? (predictionTournaments.find((t) => t.id === gameTournamentFilter)
            ?.name ?? "tournament")
        : "all-games";
    ...
    for (let i = 0; i < sortedGameLinks.length; i++) {
      ...
      const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=900x900&data=${encodeURIComponent(link)}`;
      const filename =
        `${String(i + 1).padStart(2, "0")}-${game.title}`
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/^-|-$/g, "") + "-qr.png";
      try {
    ...
    flash(
      `${sortedGameLinks.length} QR codes downloaded for ${tournamentName}.`,
    );
  }
  ...
  function parseCsvGames(text: string) {
    ...
    const rows = lines
      ...
      .map((line, index) => {
        ...
        const sport_type: "football" | "basketball" = sportRaw.includes(
          "basket",
        )
          ? "basketball"
          : "football";
        const match_label =
          get(cols, ["match_label", "label", "round", "stage"]) ||
          (sport_type === "basketball" ? "Basket" : "World Cup");
        const venue = get(cols, ["venue", "description", "desc"]);
        const rawTournamentId = get(cols, ["tournament_id"]);
        const tournament_id = UUID_RE.test(rawTournamentId)
          ? rawTournamentId
          : "";
        if (rawTournamentId && !UUID_RE.test(rawTournamentId))
          errors.push(
            `Row ${index + 2}: tournament_id "${rawTournamentId}" is not a valid UUID and will be ignored.`,
          );
        ...
        if (!kickoff)
          errors.push(
            `Row ${index + 2}: could not parse kickoff "${rawKickoff}" — use YYYY-MM-DDTHH:MM.`,
          );
    ...
    for (const row of csvPreview) {
      try {
        ...
        if (!res.ok) {
          ...
          let errMsg = "failed";
          try {
            errMsg = (JSON.parse(txt) as { error?: string }).error ?? txt;
          } catch {
            errMsg = txt;
          }
  ...
  const sortedGameLinks = useMemo(() => {
    ...
    const filtered = createdGameLinks.filter((game) => {
      if (
        gameSportFilter !== "all" &&
        game.sport.toLowerCase() !== gameSportFilter
      )
        return false;
      if (
        gameTournamentFilter !== "all" &&
        game.tournamentId !== gameTournamentFilter
      )
        return false;
      const isEnded =
        game.status.toLowerCase() === "closed" ||
        game.status.toLowerCase() === "ended";
      ...
      if (gameDateFilter === "week")
        return time >= startOfWeek && time < endOfWeek;
      if (gameDateFilter === "month")
  ...
  return (
    <main
      className="min-h-screen overflow-x-hidden bg-[#6f7c6f] text-white lg:bg-[radial-gradient(circle_at_top_left,rgba(255,214,107,0.24),transparent_28%),linear-gradient(135deg,#365665_0%,#263f49_48%,#798673_100%)]"
        ...
        <section className="min-h-screen w-full max-w-[376px] min-w-0 overflow-hidden bg-white/10 shadow-[0_26px_70px_rgba(35,54,47,0.22)] backdrop-blur-2xl sm:max-w-none sm:rounded-[30px] lg:min-h-[calc(100vh-48px)]">
          <div className="px-5 py-5 sm:px-5 sm:py-6 lg:px-8">
            ...
            <div className="mb-5 sm:mb-6">
              ...
              <div className="hidden items-center justify-between gap-4 sm:flex">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => void updateFootballScores()}
                    disabled={scoreUpdating}
                    className="rounded-full bg-white/14 px-5 py-3 text-[11px] font-black uppercase tracking-[0.16em] text-white transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50"
                  >
            ...
            <Panel className="!rounded-[28px] !bg-white/10 !p-4 pb-28 sm:pb-4">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="w-full sm:w-auto">
                  ...
                  >
                    ...
                    <svg
                      viewBox="0 0 20 20"
                      className="h-3.5 w-3.5 translate-y-[0.5px]"
                      fill="currentColor"
                      aria-hidden="true"
                    >
                      <path
                  ...
                  <div className="hidden flex-wrap items-center gap-2 sm:flex">
                    {(["all", "today", "week", "month", "ended"] as const).map(
                      (v) => (
                        <button
                          key={v}
                          type="button"
                          onClick={() => setGameDateFilter(v)}
                ...
                <div className="hidden w-full flex-wrap items-center gap-2 sm:flex sm:w-auto">
                  <select
                    ...
                    onChange={(e) => setGameSportFilter(e.target.value as any)}
                    className="h-10 flex-1 rounded-full border border-white/20 bg-white px-4 text-[11px] font-black text-[#365665] outline-none sm:flex-none"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => setGameTournamentFilter(e.target.value)}
                    className="h-10 flex-1 rounded-full border border-white/20 bg-white px-4 text-[11px] font-black text-[#365665] outline-none sm:flex-none"
                  >
              ...
              {sortedGameLinks.length === 0 ? (
                <p className="rounded-[18px] border border-white/18 bg-white/10 p-4 text-center text-[13px] font-bold text-white/70">
                      ...
                      >
                        <div className="mb-4 flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 break-words text-[15px] font-black leading-tight text-white">
                              {game.title}
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-[11px]">
                          <div>
                            ...
                            <div className="mt-1 whitespace-pre-line text-white/88">
                          ...
                          <div>
                            ...
                            <div className="mt-1 tabular-nums text-white/88">
                              {game.players}
                            </div>
                          </div>
                          <div className="col-span-2">
                            ...
                            <div className="mt-1 truncate text-white/65">
                              {game.tournamentName ?? "—"}
                            </div>
                          </div>
                        </div>
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => void copyLink(game.code)}
                            className="flex h-8 items-center justify-center rounded-full text-[10px] font-black uppercase tracking-[0.14em] text-white transition hover:bg-white/10"
                          >
                            Copy
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                              void downloadQr(game.code, game.title)
                            }
                            className="flex h-8 items-center justify-center rounded-full text-[10px] font-black uppercase tracking-[0.14em] text-white transition hover:bg-white/10"
                          >
                            QR
                    ...
                    <div className="max-h-[600px] overflow-auto">
                      {sortedGameLinks.map((game) => (
                        ...
                        >
                          ...
                          <div>
                            <span
                              className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.1em] ${game.status === "Open" ? "bg-[#ffd66b] text-[#365665]" : game.status === "Closed" ? "bg-red-500/16 text-red-100" : "bg-white/14 text-white"}`}
                            >
                              {game.status}
                            </span>
                          </div>
                          <div className="tabular-nums">{game.players}</div>
                          <div className="tabular-nums text-white/88">
                            {game.score}
                          </div>
                          ...
                          <div onClick={(e) => e.stopPropagation()}>
                            <button
                              type="button"
                              onClick={() => void copyLink(game.code)}
                              className="flex h-9 w-9 items-center justify-center rounded-full bg-white/12 text-[13px] text-white transition hover:bg-white/20"
                              title="Copy link"
                            >
                              ⧉
                            </button>
          ...
          >
            ...
            >
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] font-bold text-white/60">
                    ...
                    <span className="text-white/82">
                      home_team, away_team, sport_type, match_label, venue,
                      kickoff_at, opens_at, closes_at, tournament_id
                    </span>
                  </p>
                </div>
                <button
              ...
              <button
                ...
                onClick={() => {
                  const h =
                    "home_team,away_team,sport_type,match_label,venue,kickoff_at,opens_at,closes_at,tournament_id";
                  const e =
                    "SAGESSE,HOMENETMEN,basketball,Game 1,,2026-06-15T20:00,,,";
              ...
              {csvErrors.length > 0 && (
                <div className="mb-4 rounded-[14px] bg-red-500/16 p-3">
                  <p className="mb-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-red-300">
                    Warnings
                  </p>
              ...
              {csvPreview.length > 0 && (
                <div className="mb-5">
                  <p className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/55">
                    Preview — {csvPreview.length} game
                    {csvPreview.length !== 1 ? "s" : ""} ready to import
                  </p>
                      ...
                      <tbody>
                        {csvPreview.map((row, i) => (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 font-black text-[#ffd66b] capitalize">
                              {row.sport_type}
                            </td>
            ...
            <div className="w-full max-w-xl rounded-[30px] border border-white/18 bg-[#61716b] p-5 shadow-2xl">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 rounded-[16px] bg-white/10 p-1">
                {(["football", "basketball"] as const).map((sport) => (
                  <button
                    ...
                    className={`h-9 rounded-[13px] text-[10px] font-black uppercase tracking-[0.18em] transition ${tournamentForm.sport_type === sport ? "bg-[#ffd66b] text-[#365665]" : "text-white/72 hover:bg-white/10"}`}
                  >
              ...
              <div className="mt-5 max-h-[260px] space-y-2 overflow-auto">
                {predictionTournaments.length === 0 && (
                  <div className="rounded-[16px] bg-white/10 p-4 text-[12px] font-bold text-white/70">
                    No tournaments yet.
                  </div>
                )}
                {predictionTournaments.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center justify-between gap-3 rounded-[16px] bg-white/10 px-4 py-3"
                  >
                    <div className="min-w-0">
                      <div className="truncate text-[13px] font-black text-white">
                        {t.name}
                      </div>
                      <div className="mt-0.5 text-[10px] font-black uppercase tracking-[0.16em] text-[#ffd66b]">
                        {t.sport_type}
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setTournamentDeleteId(t.id)}
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/55 px-4 backdrop-blur-sm">
            <div className="w-full max-w-sm rounded-[26px] border border-white/18 bg-[#61716b] p-5 shadow-2xl">
              ...
              <p className="mt-2 text-[13px] font-bold leading-relaxed text-white/68">
                This will delete the tournament and remove it from linked games.
              ...
              <div className="mt-5 flex justify-end gap-2">
                ...
                <button
                  type="button"
                  onClick={() => void deleteTournament(tournamentDeleteId)}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 px-4 pb-5 backdrop-blur-sm lg:items-center lg:pb-0">
            <div className="w-full max-w-2xl rounded-[30px] border border-white/18 bg-[#61716b] p-5 shadow-2xl">
              ...
              <div className="mb-4 grid grid-cols-2 gap-2 rounded-[16px] bg-white/10 p-1">
                {(["football", "basketball"] as const).map((kind) => (
                  <button
                    ...
                    className={`h-9 rounded-[13px] text-[10px] font-black uppercase tracking-[0.18em] transition ${gameKind === kind ? "bg-[#ffd66b] text-[#365665]" : "text-white/72 hover:bg-white/10"}`}
                  >
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                ...
                <GameInput
                  label={gameKind === "basketball" ? "Team 1" : "Home Team"}
                  value={gameForm.home_team}
                  onChange={(v) =>
                    setGameForm((cur) => ({ ...cur, home_team: v }))
                  }
                />
                <GameInput
                  label={gameKind === "basketball" ? "Team 2" : "Away Team"}
                  value={gameForm.away_team}
                  onChange={(v) =>
                    setGameForm((cur) => ({ ...cur, away_team: v }))
                  }
                />
                <GameInput
                  label="Game Label"
                  ...
                  onChange={(v) =>
                    setGameForm((cur) => ({ ...cur, match_label: v }))
                  }
                />
                <GameInput
                  label="Description"
                  value={gameForm.venue}
                  onChange={(v) => setGameForm((cur) => ({ ...cur, venue: v }))}
                />
                <GameInput
                  type="datetime-local"
                  label="Match Timing"
                ...
                <GameInput
                  ...
                  onChange={(v) =>
                    setGameForm((cur) => ({ ...cur, opens_at: v }))
                  }
                />
                <GameInput
                  type="datetime-local"
                  ...
                  onChange={(v) =>
                    setGameForm((cur) => ({ ...cur, closes_at: v }))
                  }
                />
              </div>
              <button
