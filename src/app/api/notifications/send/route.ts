...
export async function POST(req: Request) {
  ...
  try {
    ...
    const matchingAudienceSubscriptions = ((allSubscriptions ?? []) as PushSubscriptionRow[])
      .filter((subscription) => audienceMatches(subscription, audience));

    const subscriptions = matchingAudienceSubscriptions
    ...
    if (subscriptions.length === 0) {
      ...
      return NextResponse.json({
        ok: true,
        message:
          matchingAudienceSubscriptions.length > 0
            ? `Notification saved, but the saved ${audience} device is missing push keys. Open the ${audience} page again and allow notifications to resubscribe.`
            : `Notification saved, but no subscribed ${audience} devices were found (${allSubscriptions?.length ?? 0} subscriptions).`,
        notification,
        sentCount: 0,
        failedCount: 0,
        subscriptionCount: 0,
        matchingAudienceSubscriptions: matchingAudienceSubscriptions.length,
