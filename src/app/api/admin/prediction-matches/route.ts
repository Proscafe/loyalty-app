...
export async function GET() {
  ...
  if (matchesError) {
    return jsonError(matchesError.message, 400);
  }
  ...
  if (ids.length > 0) {
    ...
    (entries ?? []).forEach((entry: any) => {
      ...
      entriesByMatch.set(
        entry.match_id,
        (entriesByMatch.get(entry.match_id) ?? 0) + 1,
      );
    });
  }
...
export async function POST(req: Request) {
  ...
  const tournamentId = UUID_RE.test(rawTournamentId)
    ? rawTournamentId
    : null;
  ...
  if (!kickoffAt || !opensAt || !closesAt) {
    return jsonError(
      "Match timing, open time, and close time are required.",
      400,
    );
  }
  ...
  if (tournamentId) {
    ...
    if (tournamentError) {
      console.error(
        "[prediction-matches] Tournament lookup error:",
        tournamentError.message,
      );
    } else if (!tournament) {
      console.warn(
        "[prediction-matches] Tournament not found for id:",
        tournamentId,
      );
      ...
      return jsonError(
        "Selected tournament is inactive.",
        400,
      );
    } else if (tournament.sport_type !== sportType) {
      return jsonError(
        "Selected tournament does not match this sport.",
        400,
      );
    }
  }
